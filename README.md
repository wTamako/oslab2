...
## 三、硬件的可用物理内存范围的获取方法（思考题）
如果 OS 无法提前知道当前硬件的可用物理内存范围，请问你有何办法让 OS 获取可用物理内存范围？
BIOS/UEFI接口： 操作系统可以通过BIOS（Legacy BIOS）或UEFI（现代替代品）提供的接口来获取硬件信息，包括可用的物理内存范围。这通常包括调用一些BIOS/UEFI提供的中断或使用UEFI协议来获取系统信息。例如，在UEFI中，GetMemoryMap协议可以用于获取内存映射信息。

ACPI（高级配置与电源接口）： 操作系统可以通过解析ACPI表来获取关于系统硬件的信息，包括内存配置。ACPI是一种通用的硬件配置和电源管理标准，其中包含有关硬件资源的信息，包括内存范围。



Boot Loader传递： 某些Boot Loader（引导加载程序）可以在引导操作系统时将硬件信息传递给内核。操作系统内核可以通过这些信息来获取可用的物理内存范围。
